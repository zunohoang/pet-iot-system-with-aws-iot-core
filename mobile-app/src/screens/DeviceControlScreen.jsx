...
export default function DeviceControlScreen({ route }) {
  ...
  const { reportedState, desiredState, isOnline, loading, updateDesired } =
    useDeviceShadow(deviceId);

  const relayOn      = (reportedState?.relay ?? desiredState?.relay) === 'ON';
  const isChanging   = reportedState?.relay !== desiredState?.relay && desiredState?.relay !== undefined;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.infoCard}>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.value}>{reportedState?.firmwareVersion || '—'}</Text>
        </View>
      </View>
      ...
      <View style={styles.controlSection}>
        ...
        >
          {isChanging ? (
            ...
            <>
              ...
              />
              <Text style={[styles.toggleLabel, relayOn && styles.toggleLabelOn]}>
                {relayOn ? 'ON' : 'OFF'}
              </Text>
            </>
          )}
        ...
        {!isOnline && (
          <Text style={styles.offlineHint}>Device is offline — commands will be applied when it reconnects</Text>
        )}
      </View>
      ...
      <View style={styles.shadowCard}>
        ...
        {isChanging && (
          <Text style={styles.pendingText}>Waiting for device acknowledgment...</Text>
        )}
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  infoCard:        { backgroundColor: '#1e293b', borderRadius: 14, padding: 16, marginBottom: 16 },
  ...
  controlLabel:    { fontSize: 16, fontWeight: '600', color: '#94a3b8', marginBottom: 28 },
  toggleButton:    { width: 140, height: 140, borderRadius: 70, backgroundColor: '#1e293b', justifyContent: 'center', alignItems: 'center', borderWidth: 3, borderColor: '#334155' },
  toggleOn:        { backgroundColor: '#1c1917', borderColor: '#fbbf24', shadowColor: '#fbbf24', shadowOpacity: 0.4, shadowRadius: 20, elevation: 8 },
  toggleLabel:     { fontSize: 20, fontWeight: '700', color: '#475569', marginTop: 6 },
  toggleLabelOn:   { color: '#fbbf24' },
  offlineHint:     { fontSize: 12, color: '#f59e0b', textAlign: 'center', marginTop: 16, paddingHorizontal: 24 },
  shadowCard:      { backgroundColor: '#1e293b', borderRadius: 14, padding: 16, marginTop: 8 },
  shadowTitle:     { fontSize: 13, color: '#64748b', marginBottom: 10 },
  shadowRow:       { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6 },
